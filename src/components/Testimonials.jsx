...
function TestimonialCard({ t, index }) {
  return (
    <div
      className="reveal relative bg-[#1C1C1A] border border-[#2A2A27] p-8 hover:border-[#E8631A] transition-all duration-500 group"
      style={{ transitionDelay: `${index * 100}ms` }}
    >
      {/* Quote mark */}
      <div className="absolute top-6 right-8 font-display text-[80px] leading-none text-[#2A2A27] select-none group-hover:text-[#3D3D38] transition-colors duration-300">
        "
      </div>
      ...
      <div className="flex gap-1 mb-5">
        {Array.from({ length: t.rating }).map((_, i) => (
          <span key={i} className="text-[#E8631A] text-sm">★</span>
        ))}
      </div>

      {/* Text */}
      <p className="relative z-10 text-[#A8A89E] text-sm leading-relaxed mb-8 italic">
        "{t.text}"
      </p>
...
export default function Testimonials() {
  return (
    <section id="testimonials" className="relative py-28 md:py-40 bg-[#0A0A08]">
      {/* Decorative large text */}
      <div className="absolute bottom-20 right-0 font-display text-[15vw] leading-none select-none pointer-events-none hidden lg:block"
        style={{ color: 'transparent', WebkitTextStroke: '1px rgba(232, 99, 26, 0.05)' }}
      >
        TRUST
      </div>
          ...
          <div className="flex items-center gap-4 border border-[#2A2A27] px-8 py-5 hover:border-[#E8631A] transition-colors duration-300">
            <div>
              ...
              <div className="flex gap-0.5 mt-1">
                {Array(5).fill(0).map((_, i) => <span key={i} className="text-[#E8631A]">★</span>)}
              </div>
            </div>
