    ...
    <section className="relative py-20 bg-[#E8631A] overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-0 divide-x divide-[#C0511A]">
          {stats.map((s, i) => (
            <div key={i} className="px-8 py-6 text-center reveal space-y-3">
              <div className="font-display text-6xl text-black leading-none">{s.value}</div>
              <div className="text-[#0A0A08]/70 text-sm tracking-wider uppercase">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
